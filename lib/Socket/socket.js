...
const makeSocket = (config) => {
    ...
    const DEFAULT_KEEP_ALIVE_INTERVAL_MS = 25000;
    ...
    /** connection handshake with retry logic */
    const validateConnection = async (retryCount = 0) => {
        try {
            ...
            logger.info({ browser, helloMsg, retryCount }, 'connected to WA, initiating handshake');
            ...
            logger.warn({ error: error?.message, retryCount }, 'handshake failed, attempting retry...');
            ...
            if (retryCount < 3) {
                ...
                logger.error({ error }, 'handshake failed after 3 retry attempts');
                throw error;
            }
        }
    };
